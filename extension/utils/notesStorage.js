...
export async function getAllNotesByPage() {
    const stored = await getStorageValue(PAGE_NOTES_KEY);
    return stored || {};
}
export async function getNotesForPage(url) {
    const normalizedUrl = normalizePageUrl(url);
    const allNotes = await getAllNotesByPage();
    const notes = allNotes[normalizedUrl] || [];
    return sortNotes(notes);
}
export async function saveNoteForPage(input) {
    const normalizedUrl = normalizePageUrl(input.url);
    const selectedText = input.selectedText.trim();
    const now = new Date().toISOString();
    const allNotes = await getAllNotesByPage();
    const existingNotes = allNotes[normalizedUrl] || [];
    const duplicate = existingNotes.find((note) => note.selectedText.trim() === selectedText);
    if (duplicate) {
        ...
        allNotes[normalizedUrl] = sortNotes(existingNotes.map((note) => (note.id === updatedDuplicate.id ? updatedDuplicate : note)));
...
export async function updateNoteForPage(noteId, changes) {
    const allNotes = await getAllNotesByPage();
    const now = new Date().toISOString();
    for (const [pageUrl, notes] of Object.entries(allNotes)) {
        ...
        allNotes[pageUrl] = sortNotes(notes.map((note) => (note.id === noteId ? updatedNote : note)));
        await setStorageValue(PAGE_NOTES_KEY, allNotes);
        return updatedNote;
    }
    return null;
}
export async function deleteNoteForPage(noteId) {
    const allNotes = await getAllNotesByPage();
    let changed = false;
    for (const [pageUrl, notes] of Object.entries(allNotes)) {
