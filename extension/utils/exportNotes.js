import { getAllNotesByPage } from './notesStorage.js';
function dateStamp() {
...
function notesByPageToJsonExport(notesByPage) {
...
function pageToMarkdown(url, notes) {
    const lines = [`## ${url}`, ''];
...
function notesByPageToMarkdown(notesByPage) {
    const header = [
...
export async function exportAllNotesAsMarkdown() {
    const notesByPage = await getAllNotesByPage();
